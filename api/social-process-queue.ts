...
import { buildRentadriaSocialCaption } from '../server/lib/socialCaption.js'
...
/** Maks. oglasa po jednom pozivu cron-a (izbjegava timeout na Vercelu). */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  for (let n = 0; n < MAX_PER_RUN; n++) {
    ...
    if (findErr) {
      ...
      res.status(500).json({ error: findErr.message, partial: batch })
      return
    }
    ...
    try {
      ...
      const caption = buildRentadriaSocialCaption({
        category: cat,
        titleLine,
        listingUrl,
        phoneLine,
      ...
      batch.push({ id, ok: true, warnings: warnings.length ? warnings : undefined })
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e)
      console.error('social-process', id, msg)
      await supabase
        .from('social_queue')
        .update({
          status: 'failed',
      ...
      batch.push({ id, ok: false, error: msg })
    }
  }
